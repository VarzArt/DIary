  ...
  const getCropData = (e) => {
    ...
    if (typeof cropperRef.current?.cropper !== "undefined") {
      setCropData(cropperRef.current?.cropper.getCroppedCanvas().toDataURL());
			setPost({...post, avatar: cropperRef.current?.cropper.getCroppedCanvas().toDataURL()});
    }
  };
  ...
  return (
    <div className="cropper">
      ...
      <div style={{display: 'flex', justifyContent: 'space-between', width: '80%'}}>
        ...
        >
          <h1 style={{padding: '1rem', textAlign: 'end'}}>
            Crop
          </h1>
          <img style={{ width: "335px", height: "256px", float: 'right' }} src={cropData} alt="cropped" />
        </div>
      </div>
      <br style={{ clear: "both" }} />
    </div>)
