    ...
    <div className={rootClass}>
      <div className='modal__content' onClick = {e => e.stopPropagation()}>
        ...
        <div className='modal__titleBlock'>
					Edit title:
          <MyInput name = "title" type = "text" value = {editPost.title} onChange = {e => titleHandler(e)} onBlur = {e => blurHandler(e)}/>
        </div>
				{(titleDirty && titleError) && <div className='validate__error'>{titleError}</div>}
        <div className='modal__bodyBlock'>
					Edit body:
          <MyInput name = "body" type = "text" value = {editPost.body} onChange = {e => bodyHandler(e)} onBlur = {e => blurHandler(e)}/>
        </div>
				{(bodyDirty && bodyError) && <div className='validate__error'>{bodyError}</div>}
				<div className="modal__buttons">
				<MyButton src = {trash} onClick = {() => setVisible(true)}>Delete entry</MyButton>
				<MyButton disabled = {!formValid} src = {done} onClick = {confirmEditPost}>Confirm</MyButton>
				<MyButton src = {undo} onClick = {undoChanges}>Undo Changes</MyButton>
